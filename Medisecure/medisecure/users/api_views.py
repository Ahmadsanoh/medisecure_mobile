...
class ChangePasswordView(APIView):
    ...
    def post(self, request):
        ...
        from medisecure.users.models import LogActivite
...
class MyActivityLogView(APIView):
    """Journal d'activité personnel : chaque utilisateur ne voit que ses
    propres actions (contrairement à LogActiviteListView, réservé à l'Admin
    et qui montre les actions de tous les utilisateurs)."""

    permission_classes = [IsAuthenticated]

    @extend_schema(
        responses={200: None},
        summary="Consulter mon propre journal d'activité",
        tags=["Auth"],
    )
    def get(self, request):
        from medisecure.users.models import LogActivite

        logs = LogActivite.objects.filter(utilisateur=request.user).order_by(
            "-date_action"
        )[:100]
        return Response(
            [
